...
def show_admin_dashboard():
    ...
    # Icon-based menu for navigation
    menu_items = [
        sac.MenuItem('Overview', icon='house'),
        sac.MenuItem('Candidates', icon='person'),
        sac.MenuItem('Applications', icon='file-earmark-text'),
        sac.MenuItem('Interviews', icon='calendar-event'),
        sac.MenuItem('Positions', icon='briefcase'),
        sac.MenuItem('Documents', icon='file-earmark'),
        sac.MenuItem('Messages', icon='chat-dots'),
        sac.MenuItem('Screening Tests', icon='check-square'),
        sac.MenuItem('Offer Letters', icon='envelope')
    ]
    admin_page = sac.menu(menu_items, format_func='title', open_all=True)
...
def show_candidate_dashboard(): 
    ...
    menu_items = [
        ...
        sac.MenuItem('Application', icon='file-earmark-text'),
        sac.MenuItem('Messages', icon='chat-dots'),
        sac.MenuItem('Documents', icon='file-earmark'),
        sac.MenuItem('Interviews', icon='calendar-event'),
        sac.MenuItem('Tests', icon='check-square'),
        sac.MenuItem('Offer Letters', icon='envelope')
    ]
    candidate_page = sac.menu(menu_items, format_func='title', open_all=True)

    # Company contact details in the sidebar
    with st.sidebar:
        colored_header("Contact Us", description="Get in touch with us", color_name="blue-70")
        st.write("Phone: +44 123 456 7890")
        st.write("Email: contact@miraclehealthcare.com")
        st.write("Website: www.miraclehealthcarerecruitment.co.uk")
...
def show_candidate_application():
    ...
    if application:
        ...
        # Display application as a formatted resume
        application_data = json.loads(application['application_data'])
        st.subheader("Your Submitted Application")
        st.write(f"**{application_data['personal_info']['first_name']} {application_data['personal_info']['last_name']}**")
        st.write(f"Email: {application_data['personal_info']['email']}")
        st.write(f"Phone: {application_data['personal_info']['phone']}")
        st.write(f"Address: {application_data['personal_info']['address']}")
        
        st.write("---")
        st.write("**Professional Information**")
        st.write(f"Position: {application_data['professional_info']['position']}")
        st.write(f"Years of Experience: {application_data['professional_info']['years_experience']}")
        st.write(f"Current Employer: {application_data['professional_info']['current_employer']}")
        
        st.write("---")
        st.write("**Education**")
        st.write(f"Highest Degree: {application_data['education']['highest_degree']}")
        st.write(f"Institution: {application_data['education']['institution']}")
        st.write(f"Graduation Year: {application_data['education']['graduation_year']}")
        
        st.write("---")
        st.write("**Certifications**")
        for cert in application_data['certifications']:
            st.write(f"- {cert}")
        ...
        st.write(f"Authorized to work: {'Yes' if application_data['work_authorization']['authorized'] else 'No'}")
        if application_data['work_authorization']['authorized']:
            st.write(f"Visa Type: {application_data['work_authorization']['visa_type']}")
        
        st.write("---")
        st.write("**References**")
        for ref in application_data['references']:
            st.write(f"- {ref['name']} ({ref['relation']}): {ref['contact']}")
        
        st.write("---")
        st.write("**Additional Information**")
        st.write(f"Earliest Start Date: {application_data['additional_info']['start_date']}")
        st.write(f"Preferred Schedule: {', '.join(application_data['additional_info']['preferred_schedule'])}")
        st.write(f"Willing to Relocate: {'Yes' if application_data['additional_info']['willing_to_relocate'] else 'No'}")
        ...
        with st.form("submit_application"):
            ...
            if st.form_submit_button("Submit Application"):
                if not (first_name and last_name and email and phone and position):
                    ...
                    if save_application(user_id, application_data, resume.getvalue() if resume else None, cover_letter.getvalue() if cover_letter else None):
...
def show_candidate_documents():
    ...
    if uploaded_file is not None:
        ...
        if file_size > 5:
            ...
            if any(doc['file_name'] == uploaded_file.name for doc in documents):
                st.warning("A document with this name already exists. Please rename the file or upload a different document.")
            else:
                if st.button("Upload"):
